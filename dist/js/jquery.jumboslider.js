/*!
  jQuery JumboSlider Plugin v0.1.0
        ...
        $.extend(plg, {
            ...
            dom: {
                ...
                pagination: {
                    ...
                    dot: $('\
<a href="javascript:;">\
    <svg version="1.1" class="jumboslider-pagination-mask" xmlns="http://www.w3.org/2000/svg" \
        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 88 64" \
        ...
        0-26-11.6-26-26S29.7,6,44,6s26,11.6,26,26S58.3,58,44,58z"/>\
            ...
            setup: {
                target: function() {
                    var target = plg.params && plg.params.hasOwnProperty('target') ?
            ...
            jumboslider: {
                ...
                createArrows: function() {
                    var obj = this,
                        arrows = obj.arrows = plg.dom.arrows.clone();
                ...
                createPagination: function() {
                    var obj = this,
                        pagination = obj.pagination = plg.dom.pagination.block.clone();

                    $.extend(pagination, {
                        init: function() {
                            var pagination = this,
                                current = plg.dom.pagination.current.clone();

                            for (var i = 0; i < obj.items.length; i++) {
                                var dot = plg.dom.pagination.dot.clone();
                                dot.attr('rel', i+1);
    ...
    target.each(function() {
        if (!this.jumboslider) {
            $.extend(this, {
                jumboslider: $.extend($(this), plg.jumboslider)
            });

            var jumboslider = this.jumboslider;

            plg.clients.push(jumboslider.init());
        }
    });

    // Extend target with JumboSlider methods (if not yet)
    if (!target.jumboMethods) {
        $.extend(target, {
            jumboMethods: true,
            slideNext: function() {
                this.each(function() {
